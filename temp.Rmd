...
```{r}
quartiles.1 <- quantile(d.1[d.1$tobacco_smoking_status == "Current Reformed Smoker for < or = 15 yrs",]$count, probs = c(0.25, 0.75))
quartiles.1
low.1 <- rownames(d.1[d.1$tobacco_smoking_status == "Current Reformed Smoker for < or = 15 yrs" & d.1$count <= quartiles.1[1],])
high.1 <- rownames(d.1[d.1$tobacco_smoking_status == "Current Reformed Smoker for < or = 15 yrs" & d.1$count >= quartiles.1[2],])
```
...
We can visibly see ~3 clusters, above PC2 is mostly non-smokers, below it we have both.
...
# Plot PCA results
qplot(pcaResults$x[,1], pcaResults$x[,2], 
      col=colData(pca_dds.symbol)$pack_years_smoked, 
      size=I(2), alpha=I(0.6)) +
  labs(x="PC-1", y="PC-2", color="Packs Per Year") +
  theme_minimal()
```
...
```{r}
quartiles.3 <- quantile(d.3[d.3$tobacco_smoking_status == "Current Reformed Smoker for < or = 15 yrs",]$count, probs = c(0.25, 0.75))
quartiles.3
low.3 <- rownames(d.3[d.3$tobacco_smoking_status == "Current Reformed Smoker for < or = 15 yrs" & d.3$count <= quartiles.1[1],])
high.3 <- rownames(d.3[d.3$tobacco_smoking_status == "Current Reformed Smoker for < or = 15 yrs" & d.3$count >= quartiles.3[2],])
...
filter.intersect <- rbind(filter.sgn.genes.1.nona, filter.sgn.genes.2.nona, filter.sgn.genes.3.nona)

# Remove duplicates if necessary
filter.intersect <- filter.intersect[!duplicated(filter.intersect$symbol),]
filter.intersect
```
