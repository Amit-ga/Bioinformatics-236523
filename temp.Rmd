...
* We save the lower quartile and the upper quartile, and the samples that correspond to them, for the survival analysis later
```{r}
write.csv(low.2, file = "WSCD2.low2.csv", row.names = FALSE)
write.csv(high.2, file = "WSCD2.high2.csv", row.names = FALSE)
```
## Visualization of gene expression for multiple genes
...
* We save the lower quartile and the upper quartile, and the samples that correspond to them, for the survival analysis later
```{r}
write.csv(low.3, file = "ATP2A1.low3.csv", row.names = FALSE)
write.csv(high.3, file = "ATP2A1.high3.csv", row.names = FALSE)
```
...
gbm.clin <- GDCquery_clinic("CPTAC-3", "clinical")
View(gbm.clin)
```
...
gbm.clin.df$deceased = gbm.clin.df$vital_status == "Dead"

# create an "overall survival" variable that is equal to days_to_death
# for dead patients, and to days_to_last_follow_up for patients who
# are still alive
gbm.clin.df$overall_survival = ifelse(gbm.clin.df$deceased,
                                   gbm.clin.df$days_to_death,
                                   gbm.clin.df$days_to_last_follow_up)

# convert overall_survival to numeric for Surv object
gbm.clin.df$overall_survival <- as.numeric(as.character(gbm.clin.df$overall_survival))
```
- Form a dependent variable out of the overall_survival and deceased information:
```{r}
lung.Surv <- Surv(gbm.clin.df$overall_survival, gbm.clin.df$deceased)
```
...
- For doing so, we will divide the patients into 2 groups: Patients who smoke over 5 cigarettes per day, and Patients who smoke under 5 cigarettes per day.
```{r}
# handle 'NAs' in Lifelong Non-Smoker 
gbm.clin.df <- gbm.clin.df %>%
...
```{r}
gbm.clin.df <- gbm.clin.df %>%
  mutate(more_than_5_cigarettes_pd = case_when(
    cigarettes_per_day < 5 ~ "NO",
    cigarettes_per_day >= 5 ~ "YES",
...
ggsurvplot(
  fit=survfit(lung.Surv ~ more_than_5_cigarettes_pd, data=gbm.clin.df),
  data=gbm.clin.df,
  xlab = "Days",
  legend.title = "cigarettes per day",
  legend.labs = c("less than 5", "more than 5"),
  pval=T,
  risk.table=T,
...
gbm.clin.df$ajcc_pathologic_stage = gsub("[123]$", "", gbm.clin.df$ajcc_pathologic_stage)
...
gbm.clin.df$ajcc_pathologic_stage = gsub("[ABC]$", "", gbm.clin.df$ajcc_pathologic_stage)

# we remove those with stage "not reported", since they are unknown
gbm.clin.df[which(gbm.clin.df$ajcc_pathologic_stage == "Not Reported"), "ajcc_pathologic_stage"] = NA

# finally, we also remove those with tumor stage 4, since they are too few
gbm.clin.df[which(gbm.clin.df$ajcc_pathologic_stage == "Stage IV"), "ajcc_pathologic_stage"] = NA

table(gbm.clin.df$ajcc_pathologic_stage)
```

```{r}
ggsurvplot(
  fit=survfit(lung.Surv ~ ajcc_pathologic_stage, data=gbm.clin.df),
  data=gbm.clin.df,
...
```
Add conclusion here: ------------------------------------------
### Survival Analysis on groups: Lifelong non-smoker vs.Current reformed smoker for <= 15 years
...
- Take only GPR15 high/low samples 
```{r}
gbm.clin.has1 <- gbm.clin[gbm.clin$submitter_id %in% c(gbm.data@colData[high,]$submitter_id,
                                                       gbm.data@colData[low,]$submitter_id),]
gbm.clin.GPR15 <- gbm.clin[gbm.clin$submitter_id %in% c(gbm.data@colData[GPR15.high,]$submitter_id,
                                                       gbm.data@colData[GPR15.low,]$submitter_id),]
```
...
gbm.clin.GPR15$expression <- ifelse(gbm.clin.GPR15$submitter_id %in% gbm.data@colData[GPR15.high,]$submitter_id,
                                   "high", "low")
```
...
TCGAanalyze_survival(
    ...
    main = "Survival analysis of Glioblastoma\npatients by GPR15 expression",
    height = 10,
    width=10,
    filename = "~/Documents/intro_to_bioinformatics/winter2324/tutorial_9/KM_curv_HAS1.pdf")
    filename = "KM_curv_GPR15.pdf")
```

# doi:10.1007/s12307-019-00224-2
### Survival Analysis on groups: Current smoker vs. Current reformed smoker for <= 15 years

- Read WSCD2 low, high samples from the second analysis 
```{r}
WSCD2.low <- read.csv("WSCD2.low2.csv", header = TRUE)$x
WSCD2.high <- read.csv("WSCD2.high2.csv", header = TRUE)$x
```

- Take only WSCD2 high/low samples 
```{r}
gbm.clin.WSCD2 <- gbm.clin[gbm.clin$submitter_id %in% c(gbm.data@colData[WSCD2.high,]$submitter_id,
                                                       gbm.data@colData[WSCD2.low,]$submitter_id),]
```

- Add a column that indicates which samples is high/low
```{r}
gbm.clin.WSCD2$expression <- ifelse(gbm.clin.WSCD2$submitter_id %in% gbm.data@colData[WSCD2.high,]$submitter_id,
                                   "high", "low")
```
...
TCGAanalyze_survival(
    data = gbm.clin.WSCD2,
    clusterCol = "expression",
    main = "Survival analysis of Glioblastoma\npatients by WSCD2 expression",
    height = 10,
    width=10,
    filename = "KM_curv_WSCD2.pdf")
```

### Survival Analysis on groups: Current reformed smoker for <= 15 years vs. Current reformed smoker for > 15 years

- Read ATP2A1 low, high samples from the third analysis 
```{r}
ATP2A1.low <- read.csv("ATP2A1.low3.csv", header = TRUE)$x
ATP2A1.high <- read.csv("ATP2A1.high3.csv", header = TRUE)$x
```

- Take only ATP2A1 high/low samples 
```{r}
gbm.clin.ATP2A1 <- gbm.clin[gbm.clin$submitter_id %in% c(gbm.data@colData[ATP2A1.high,]$submitter_id,
                                                       gbm.data@colData[ATP2A1.low,]$submitter_id),]
```

- Add a column that indicates which samples is high/low
```{r}
gbm.clin.ATP2A1$expression <- ifelse(gbm.clin.ATP2A1$submitter_id %in% gbm.data@colData[ATP2A1.high,]$submitter_id,
                                   "high", "low")
```
...
```{r}
TCGAanalyze_survival(
    data = gbm.clin.ATP2A1,
    clusterCol = "expression",
    main = "Survival analysis of Glioblastoma\npatients by ATP2A1 expression",
    height = 10,
    width=10,
    filename = "KM_curv_ATP2A1.pdf")
```
